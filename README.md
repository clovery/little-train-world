...
- 单屏驾驶舱布局：大幅游戏画面、画面内旅程 HUD、底部驾驶台，以及车库和路线选择抽屉。
- 带加加速度限制的启动与制动、自动进站减速、车体悬挂反馈、全车型独立轮组动画、三档游玩速度、鸣笛和车门动画。
- 白天与夜晚切换、车灯、动态云层，以及随田野、森林、海岸、沙漠和雪山变化的环境粒子与前景视差。
- 随速度和牵引力变化的低频引擎声、制动火花、强化蒸汽、中文语音科普、静音和到站重开。
...
- `src/game/motion.ts`：加加速度受限的牵引/制动模型和进站制动速度计算。
- `src/game/wheelLayouts.ts`：9 种车型的轮轴位置、轮径与轮子纹理配置。
...
- `tests/motion.test.ts`：启动、制动和进站速度模型测试。
- `tests/wheelLayouts.test.ts`：全车型轮组覆盖和纹理裁切边界测试。
